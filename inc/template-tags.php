...
function fastr_post_nav() {
	...
	<nav class="navigation post-navigation" role="navigation">
		<h1 class="screen-reader-text"><?php _e( 'Post navigation', 'fastr' ); ?></h1>
		<div class="nav-links">
		...
		<article id="div-comment-<?php comment_ID(); ?>" class="comment-body">
			<footer class="comment-meta">
				<div class="comment-author vcard">
					...
					<?php printf( __( '%s <span class="says">says:</span>', 'fastr' ), sprintf( '<cite class="fn">%s</cite>', get_comment_author_link() ) ); ?>
				</div><!-- .comment-author -->
 ...
 */
function fastr_categorized_blog() {
	...
	if ( '1' != $all_the_cool_cats ) {
		// This blog has more than 1 category so fastr_categorized_blog should return true.
		return true;
	} else {
		// This blog has only 1 category so fastr_categorized_blog should return false.
		return false;
	}
}

/**
 * Flush out the transients used in fastr_categorized_blog.
 */
function fastr_category_transient_flusher() {
	// Like, beat it. Dig?
	delete_transient( 'all_the_cool_cats' );
}
add_action( 'edit_category', 'fastr_category_transient_flusher' );
add_action( 'save_post',     'fastr_category_transient_flusher' );
